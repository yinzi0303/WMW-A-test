...
Run_BSDE <- function(logcount_data, metas){
  
  
  diagnosis = as.factor(metas$diagnosis)
  diagnosis2=matrix("1",ncol=1,nrow=length(diagnosis))
  diagnosis2[which(diagnosis == "Control")] = "0"
  diagnosis= apply(diagnosis2, 1,as.numeric)
  
  op_pvalw=matrix(ncol=1,nrow=nrow(logcount_data))
  
  for(ig in 1:nrow(logcount_data)){
    
    logdata=logcount_data[ig,]
    individual=as.factor(metas$individual)
    # diagnosis=metas$diagnosis
    cur_cluster = as.factor(metas$cluster)
    
    tempa=tryCatch({cal_w2_pval(count_per_gene=logdata,meta_individual=individual,
                                meta_phenotype=diagnosis)}, 
                   error = function(e) {NA} )
    op_pvalw[ig]=tryCatch({tempa[[1]]}, error = function(e) {NA} )
  }
  
  rownames(op_pvalw) = rownames(logcount_data)
