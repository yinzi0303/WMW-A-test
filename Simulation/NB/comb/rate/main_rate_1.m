    ...
    for i = 1:length(n_control) %case/control sample
        ...
        for j = 1:T %trial times
            ...
            for k = 1:length(K) %auxiliary sample size
                for g = 1:11%1:11
                    z = [nbinrnd(100,0.5,[1,fix(K(k)*N*(g*0.1-0.1))]) nbinrnd(100,D(d),[1,K(k)*N-fix(K(k)*N*(g*0.1-0.1))])];
                    idx = intersect(find(z>=allmin),find(z<=allmax));
                    z = z(idx);
                    z = z(:);
                    [pwmwa1(d,i,k,g,j),pwmwa2(d,i,k,g,j)]=wmwa(x,y,z,nperm);
                end
            end
        end
...
% save('resultn5_12.mat', 'power_wmwa1', 'power_wmwa2', 'N', 'T', 'D', 'K');
save('result_comb_rate_1.mat', 'power_wmwa1', 'power_wmwa2','power_wmw', 'power_t', 'power_wc', 'power_tR', 'N', 'T', 'D', 'K');