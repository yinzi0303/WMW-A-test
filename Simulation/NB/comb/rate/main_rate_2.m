    ...
    for i = 1:length(n_control) %case/control sample
        ...
        for j = 1:T %trial times
            ...
            for k = 1:length(K) %auxiliary sample size
                for g = 1:11%1:11
                    z = [nbinrnd(100,0.5,[1,fix(K(k)*N*(g*0.1-0.1))]) nbinrnd(D(d),0.5,[1,K(k)*N-fix(K(k)*N*(g*0.1-0.1))])];
...
save('result_comb_rate_2.mat', 'power_wmwa1', 'power_wmwa2','power_wmw', 'power_t', 'power_wc', 'power_tR', 'N', 'T', 'D', 'K');