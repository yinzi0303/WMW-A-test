...
%                 z = [gamrnd(0.5,1,[1,K(k)*n]) gamrnd(D(d),1,[1,K(k)*n])];
                z = [nbinrnd(100,0.5,[1,K(k)*n]) nbinrnd(100,D(d),[1,K(k)*n])];
        ...
        for k = 1:length(K)
            ...
            %             power_wmwa_asy(d,i,k) = mean(rmmissing(a));
...
save("result_asy_mneq.mat", 'power_wmwa_asy', 'power_wmwa1', 'power_wmwa2', 'N', 'T', 'D', 'K');